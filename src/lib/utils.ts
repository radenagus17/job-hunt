...
import { CategoryJobType, CompanyType, JobTypes, optionType } from "@/types";
  ...
  if (!isLoading && !error && data) {
    return await Promise.all(
      data.map(async (item: any) => {
        ...
        if (imageName) imageUrl = await supabasePublicUrl(imageName, "company");
        else imageUrl = "/images/company.png";
